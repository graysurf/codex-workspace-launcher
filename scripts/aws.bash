...
_aws_detect_gh_target_for_create() {
  ...
  local gh_owner_repo=""
  if [[ -n "$candidate" ]]; then
    read -r gh_host gh_owner_repo < <(_aws_parse_repo_spec "$candidate" "$gh_host" 2>/dev/null || printf '%s %s\n' "$gh_host" "")
  fi
...
aws() {
  ...
  if [[ "$auth_mode" != "none" && "$auth_mode" != "env" ]]; then
    if [[ -z "$env_token" && ( "$subcmd" == "create" || "$subcmd" == "reset" || ( "$subcmd" == "auth" && "$provider" == "github" ) ) ]]; then
      ...
      if (( want_help == 0 )); then
        ...
        local gh_owner_repo=""
        if [[ "$subcmd" == "create" ]]; then
          read -r gh_host gh_owner_repo < <(_aws_detect_gh_target_for_create "${@:2}" 2>/dev/null || printf '%s %s\n' "$gh_host" "")
        elif [[ "$subcmd" == "auth" && "$provider" == "github" ]]; then
          gh_host="$(_aws_detect_gh_host_for_auth_github "${@:3}" 2>/dev/null || printf '%s\n' "$gh_host")"
        fi
  ...
  local -a extra_args=()
  local decl=""
  if decl="$(declare -p AWS_DOCKER_ARGS 2>/dev/null)"; then
    if [[ "$decl" == "declare -a"* ]]; then
      extra_args=("${AWS_DOCKER_ARGS[@]}")
    elif [[ -n "${AWS_DOCKER_ARGS:-}" ]]; then
      read -r -a extra_args <<<"${AWS_DOCKER_ARGS}"
    fi
  elif [[ -n "${AWS_DOCKER_ARGS:-}" ]]; then
    read -r -a extra_args <<<"${AWS_DOCKER_ARGS}"
  fi
  ...
  if [[ -n "$injected_token" ]]; then
    if [[ -n "$injected_gpg_key" ]]; then
      GH_TOKEN="$injected_token" GITHUB_TOKEN="" AGENT_WORKSPACE_GPG_KEY="$injected_gpg_key" docker "${run_args[@]}" "$image" "$@"
    else
      GH_TOKEN="$injected_token" GITHUB_TOKEN="" docker "${run_args[@]}" "$image" "$@"
    fi
  else
    if [[ -n "$injected_gpg_key" ]]; then
      AGENT_WORKSPACE_GPG_KEY="$injected_gpg_key" docker "${run_args[@]}" "$image" "$@"
    else
...
_aws_workspaces() {
  ...
  docker ps -a --filter label=agent-kit.workspace=1 --format '{{.Names}}' 2>/dev/null || true
}
...
_aws_complete() {
  ...
  if [[ $COMP_CWORD -eq 1 ]]; then
    _aws_compgen_words "auth create ls exec rm reset tunnel -h --help" "$cur"
    return 0
  fi
  ...
  case "$subcmd" in
    auth)
      ...
      if [[ $COMP_CWORD -eq 2 ]]; then
        _aws_compgen_words "codex github gpg -h --help" "$cur"
        return 0
      fi

      if [[ "$prev" == "--container" || "$prev" == "--name" ]]; then
        _aws_compgen_words "$(_aws_workspaces)" "$cur"
        return 0
      fi
    ...
    create)
      ...
      _aws_compgen_words "--no-extras --private-repo --no-work-repos --name -h --help" "$cur"
      return 0
      ;;
    ls)
      _aws_compgen_words "-h --help" "$cur"
      return 0
      ;;
    exec)
      if [[ "$prev" == "--user" ]]; then
    ...
    rm)
      ...
      if [[ "$cur" == -* ]]; then
        _aws_compgen_words "--all --yes -h --help" "$cur"
    ...
    reset)
      if [[ $COMP_CWORD -eq 2 ]]; then
        _aws_compgen_words "repo work-repos opt-repos private-repo -h --help" "$cur"
        return 0
      fi
    ...
    tunnel)
      ...
      if [[ -z "$container2" ]]; then
        if [[ "$cur" == -* ]]; then
          _aws_compgen_words "--detach --name -h --help" "$cur"
        else
          _aws_compgen_words "$(_aws_workspaces)" "$cur"
        fi
      else
...
complete -F _aws_complete aws aw
